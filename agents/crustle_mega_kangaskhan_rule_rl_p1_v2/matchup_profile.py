...
DRAGAPULT_VISIBLE_IDS = {119, 120, 121, 235, 1071}
LUCARIO_VISIBLE_IDS = {675, 676, 677, 678, 673, 674}
...
def detect_matchup_profile(obs) -> MatchupProfile:
    ...
    visible_ids = {getattr(card, "id", None) for card in visible_cards if card is not None}

    if "dragapult" in names or visible_ids & DRAGAPULT_VISIBLE_IDS:
    ...
    if "lucario" in names or visible_ids & LUCARIO_VISIBLE_IDS:
        return MatchupProfile(
            name="mega_lucario",
            prefers_crustle_wall=True,
