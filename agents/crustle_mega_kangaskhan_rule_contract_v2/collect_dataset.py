...
def _compute_shaped_reward(previous_observation, next_observation, player_index: int) -> float:
    ...
    try:
        prev_snapshot, prev_plan = build_turn_plan(previous_observation)
        next_snapshot, next_plan = build_turn_plan(next_observation)
        prev_deck_state = build_state_view(prev_snapshot, prev_plan)
        next_deck_state = build_state_view(next_snapshot, next_plan)
    except Exception:
