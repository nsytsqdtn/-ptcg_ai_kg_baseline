...
This package keeps the same compact three-plan structure. Changes from compact_v1_1:

- PrizePlan now treats Boss/Lisia/Switch routes as confirmed only when the corresponding legal action exists in the current action list.
- Bench attackers no longer create confirmed PrizePlan routes unless a legal switch/retreat action exists. Bench attach + switch lines remain possible, not confirmed.
- Boss/Lisia in hand is no longer enough to claim a same-turn gust route after a Supporter has already been used.
- Fixed generic TO_HAND prize selection: it no longer searches for the opponent target card ID; it now searches for route components such as energy, Switch, Boss/Lisia, or the planned attacker.
