...
def choose_to_hand(obs, actions, selected_plan, setup, wall, prize):
    ...
    for a in actions:
        ...
        if selected_plan == "setup":
            ...
            # Do not use prize.target_card_id here: it is the opponent target,
            # not a card we should search into hand. Search choices should find
            # route components: attacker, energy, switch, or gust pieces.
            if prize.need_energy and _is_energy_for_prize_attacker(cid, prize):
                score = 980.0
            elif prize.need_switch and cid == CardIds.SWITCH:
                score = 820.0
            elif prize.route == "boss" and cid == CardIds.BOSS_ORDERS:
                score = 880.0
            elif prize.route == "lisia" and cid == CardIds.LISIA:
                score = 860.0
            elif prize.route == "petrel_to_boss" and cid in {CardIds.BOSS_ORDERS, CardIds.LISIA}:
                score = 760.0
            elif cid == prize.attacker_card_id:
                score = 520.0
            elif cid in {CardIds.GROW_GRASS_ENERGY, CardIds.BASIC_GRASS, CardIds.MIST_ENERGY, CardIds.SPIKY_ENERGY} and prize.attacker_card_id is not None:
                score = 360.0
        else:
...
def _is_energy_for_prize_attacker(cid: int | None, prize) -> bool:
    if cid is None or not prize.need_energy:
        return False
    if prize.attacker_card_id == CardIds.CRUSTLE:
        # Crustle needs a Grass source when the plan explicitly asks for energy.
        return cid in {CardIds.GROW_GRASS_ENERGY, CardIds.BASIC_GRASS}
    if prize.attacker_card_id == CardIds.MEGA_KANGASKHAN_EX:
        return cid in ENERGY_IDS
    return cid in ENERGY_IDS
