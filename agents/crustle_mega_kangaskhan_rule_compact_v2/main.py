...
from ck_compact_core.wall_plan import build_wall_plan

DECK_PATH = AGENT_DIR / "deck.csv"
DEBUG_PATH = Path(os.environ.get("RULE_DEBUG_PATH", str(AGENT_DIR / "compact_debug.jsonl")))
...
def agent(obs_dict: dict, configuration=None) -> list[int]:
    ...
    try:
        ...
        wall_plan = build_wall_plan(state)
        prize_plan = build_prize_plan(state, actions)
        selected_plan = choose_plan(state, setup_plan, wall_plan, prize_plan)
        scored = score_actions(actions, state, selected_plan, setup_plan, wall_plan, prize_plan)

        if state.phase == "SELECT":
            selected = choose_context_action(obs, state, actions, selected_plan, setup_plan, wall_plan, prize_plan, scored=scored)
        else:
            selected = choose_best(scored, state)

        selected = _normalize_selection(obs, selected, scored)
        write_debug(DEBUG_PATH, obs, state, setup_plan, wall_plan, prize_plan, selected_plan, scored, selected)
        return selected
    except Exception:
