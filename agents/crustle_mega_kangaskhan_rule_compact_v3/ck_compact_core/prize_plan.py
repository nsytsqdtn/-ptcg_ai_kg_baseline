...
class PrizePlan:
    ...
    breaks_defense: bool = False
    wins_game: bool = False
    reason: str = "no_prize_plan"

    # Non-KO pressure is intentionally secondary. It is used only inside TempoPlan
...
def build_prize_plan(state, actions=None) -> PrizePlan:
    """Build a same-turn prize plan from the *current legal actions*.

    Compact v1.2 deliberately treats hand contents as insufficient proof. A
    Boss/Lisia/Switch route is confirmed only when the corresponding legal action
    is present now. This prevents the plan selector from entering PrizePlan when
    the required card is in hand but cannot legally be played this turn.
    """
    actions = actions or []
    ...
    for attacker in attackers:
        ...
        if damage <= 0:
            continue
        breaks_defense = bool(state.crustle_active and attacker_id != CardIds.CRUSTLE and need_switch)

        # Direct active target does not need gust. An active attacker that only
        # needs one legal attachment is a confirmed two-step line: attach now,
        # then attack in the next decision of the same turn.
        if state.opponent_active_view is not None and damage >= state.opponent_active_view.hp:
            ...
            candidates.append(PrizePlan(
                ...
                breaks_defense=breaks_defense,
                wins_game=prize >= state.my_prizes_left and confidence == "confirmed",
                reason=_reason("direct_ko", need_switch, need_energy),
            ))

        # Bench targets require a currently legal gust route. Petrel is itself a
        # Supporter, so Petrel -> Boss/Lisia is not a confirmed same-turn route;
        # keep it possible/future pressure only if Petrel is currently legal.
        for target in state.opponent_targets:
            if target.slot != "bench" or damage < target.hp:
                continue
            route, gust_confidence = _gust_route_for_target(actions, target, state)
            if route == "none":
                continue
            ...
            candidates.append(PrizePlan(
                ...
                breaks_defense=breaks_defense,
                wins_game=prize >= state.my_prizes_left and confidence == "confirmed",
                reason=_reason(f"{route}_bench_ko", need_switch, need_energy),
            ))

    if not candidates:
        return best_pressure
    best = max(candidates, key=_prize_plan_value)
    # Preserve the pressure hint for TempoPlan even when a speculative KO route
...
def _candidate_attackers(state, actions) -> list[tuple[int, str, str, int, bool, bool, str]]:
    ...
    has_switch = _has_legal_switch_action(actions)
    ...
    for idx, card in enumerate(state.bench):
        ...
        if cid == CardIds.CRUSTLE:
            if energy_count(card) >= 3:
                # A ready bench attacker is confirmed only when a switch/retreat
                # action is legal now. Otherwise it is a future/possible pressure
                # line and must not select PrizePlan by itself.
                out.append((CardIds.CRUSTLE, "Crustle", f"bench:{idx}", 120, True, False, "confirmed" if has_switch else "possible"))
                ...
                # Attach + switch from bench is intentionally possible, not
                # confirmed. It is a multi-step line and should not override a
                # stable wall unless later decisions make each step legal.
                out.append((CardIds.CRUSTLE, "Crustle", f"bench:{idx}", 120, True, True, "possible"))
        elif cid == CardIds.MEGA_KANGASKHAN_EX:
            if energy_count(card) >= 3:
                out.append((CardIds.MEGA_KANGASKHAN_EX, "Mega Kangaskhan ex", f"bench:{idx}", 200, True, False, "confirmed" if has_switch else "possible"))
...
def _has_legal_switch_action(actions) -> bool:
    return any(action.has("switch") for action in actions or [])


def _has_legal_card_action(actions, card_id: int) -> bool:
    return any(action.card_id == card_id for action in actions or [])


def _gust_route_for_target(actions, target, state) -> tuple[str, str]:
    if _has_legal_card_action(actions, CardIds.BOSS_ORDERS):
        return "boss", "confirmed"
    if target.is_basic and _has_legal_card_action(actions, CardIds.LISIA):
        return "lisia", "confirmed"
    if _has_legal_card_action(actions, CardIds.PETREL):
        # Petrel can find Boss/Lisia for a future turn, but cannot also play it
        # this turn. Only expose a possible route if the target card is not known
        # dead in deck knowledge.
...
def _build_pressure_hint(state, attackers) -> PrizePlan:
    ...
    for attacker_id, _name, _slot, damage, _need_switch, need_energy, _confidence in attackers:
        ...
        # itself, but TempoPlan may use it when otherwise waiting.
        if ratio >= 0.55:
...
def _reason(base: str, need_switch: bool, need_energy: bool) -> str:
    parts = []
    if need_energy:
        parts.append("attach")
    if need_switch:
        parts.append("switch")
    parts.append(base)
    return "_then_".join(parts) if len(parts) > 1 else base
