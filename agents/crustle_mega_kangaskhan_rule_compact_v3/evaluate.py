...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate the compact rule agent.")
    ...
    parser.add_argument("--opponents", type=str, default="mega_lucario_beginner,dragapult_rule_based,alakazam_rule_based,crustle_aware_fighting_agent,multiply_agent_best_940")
...
def run_evaluation(agent_name: str, games: int, label: str, output: Path | None = None, progress_every: int = 5, opponents: list[str] | None = None) -> dict:
    run_at = datetime.now(UTC).isoformat()
    matchups = {}
    game_records = []
    if opponents is None:
        opponents = ["mega_lucario_beginner", "dragapult_rule_based", "alakazam_rule_based", "crustle_aware_fighting_agent", "multiply_agent_best_940"]
...
def main():
    ...
    report = run_evaluation(args.agent, args.games, args.label, args.output, progress_every=args.progress_every, opponents=opponents)
