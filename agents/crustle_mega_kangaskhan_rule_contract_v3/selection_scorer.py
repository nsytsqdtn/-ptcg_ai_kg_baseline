...
def _plan_alias(primary_plan: str) -> str:
    return {
        "finish": "close_game",
...
def score_petrel_play(deck_state, state, hand_ids: set[int] | None = None, discard_ids: set[int] | None = None) -> tuple[float, str | None]:
    ...
    primary_plan = _plan_alias(getattr(deck_state, "primary_plan", "stabilize"))
    if (
...
def score_petrel_target(card_id: int, deck_state) -> tuple[float, str | None]:
    primary_plan = _plan_alias(getattr(deck_state, "primary_plan", "stabilize"))
    if getattr(deck_state, "must_bench_basic", False) or primary_plan == "survival_setup":
    ...
    if primary_plan in {"setup_kangaskhan", "tank_and_heal"}:
...
def score_hilda_target(
    ...
    if card_id == CardIds.CRUSTLE and (primary_plan in {"setup_crustle", "wall_and_tax"}):
        return 105.0, "hilda_crustle"
    if card_id == CardIds.DWEBBLE and deck_state.setup_missing_crustle:
        return 95.0, "hilda_dwebble"
    if card_id == CardIds.MEGA_KANGASKHAN_EX and primary_plan in {"setup_kangaskhan", "tank_and_heal"}:
        return 90.0, "hilda_kang"
    if card_id == CardIds.GROW_GRASS_ENERGY and primary_plan in {"setup_crustle", "wall_and_tax"}:
        return 92.0, "hilda_grow_grass"
    if card_id == CardIds.MIST_ENERGY and matchup.values_mist_energy:
        return 88.0, "hilda_mist"
    if card_id == CardIds.SPIKY_ENERGY and primary_plan == "tank_and_heal":
...
def score_poffin_target(card_id: int, deck_state, matchup) -> tuple[float, str | None]:
    ...
    if card_id == CardIds.DWEBBLE:
        if primary_plan in {"setup_crustle", "wall_and_tax"} or matchup.prefers_crustle_wall:
            return 104.0, "poffin_dwebble"
        return 78.0, "poffin_dwebble_fallback"
    if card_id == CardIds.MEGA_KANGASKHAN_EX:
        if primary_plan in {"setup_kangaskhan", "tank_and_heal", "stabilize"}:
...
def score_switch_target(card_id: int, deck_state) -> tuple[float, str | None]:
    primary_plan = _plan_alias(getattr(deck_state, "primary_plan", "stabilize"))
    if primary_plan == "wall_and_tax" and card_id == CardIds.CRUSTLE:
        return 120.0, "switch_crustle_wall"
    if primary_plan in {"tank_and_heal", "setup_kangaskhan"} and card_id == CardIds.MEGA_KANGASKHAN_EX:
        return 105.0, "switch_kang"
    if primary_plan == "setup_crustle" and card_id == CardIds.DWEBBLE:
