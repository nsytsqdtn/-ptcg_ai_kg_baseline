...
from turn_plan import build_turn_plan


def _legacy_plan_alias(plan: str) -> str:
    return {
        "finish": "close_game",
        "setup_backup": "survival_setup",
        "setup_crustle_wall": "setup_crustle",
        "preserve_wall": "wall_and_tax",
        "protect_bench_core": "protect_bench_vs_dragapult",
        "kang_engine": "tank_and_heal",
    }.get(plan, plan)
...
def _phase_for_plan(plan: str) -> str:
    plan = _legacy_plan_alias(plan)
    if plan == "close_game":
        return "close_game"
    if plan in {"survival_setup", "setup_crustle_wall_now", "setup_crustle", "setup_kangaskhan"}:
        return "setup"
    if plan == "prevent_loss":
        return "prevent_loss"
    if plan == "wall_and_tax":
        return "wall_lock"
    if plan == "kang_engine":
        return "tank_cycle"
    if plan in {"disruption_loop"}:
        return "disruption"
    return "midgame"
...
def analyze_deck_state(obs, deck_knowledge=None) -> DeckState:
    snapshot, plan = build_turn_plan(obs, deck_knowledge)
    ...
    legacy_plan = _legacy_plan_alias(plan.mode)

    plan_scores = {
        "finish": 100.0 if plan.mode == "finish" else 0.0,
        "close_game": 100.0 if plan.mode == "finish" else 0.0,
        "survival_setup": 90.0 if plan.mode == "setup_backup" else 0.0,
        "prevent_loss": 80.0 if plan.mode == "prevent_loss" else 0.0,
        "setup_crustle_wall_now": 75.0 if plan.mode == "setup_crustle_wall" and "wall_can_be_made" in plan.reasons else 0.0,
        "wall_and_tax": 70.0 if plan.mode == "preserve_wall" else 0.0,
        "setup_crustle": 65.0 if plan.mode == "setup_crustle_wall" else 0.0,
        "kang_engine": 55.0 if plan.mode == "kang_engine" else 0.0,
        "tank_and_heal": 55.0 if plan.mode == "kang_engine" else 0.0,
        "setup_kangaskhan": 50.0 if plan.mode == "kang_engine" else 0.0,
        "disruption_loop": 42.0 if plan.mode == "preserve_wall" and disruption_window else 0.0,
        "stabilize": 35.0 if plan.mode == "stabilize" else 0.0,
    }
    ...
    tags = [plan.mode, legacy_plan] + list(plan.reasons) + snapshot.matchup.tags
    ...
    return DeckState(
        phase=_phase_for_plan(plan.mode),
        primary_plan=legacy_plan,
