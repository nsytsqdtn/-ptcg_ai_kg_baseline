...
_cur_turn_logs = []
_fallback_count = 0
_last_decision_debug = {}
_training_decisions = []

AGENT_NAME = "archaludon_rl_v1"
FEATURE_DIM = 192
RL_WEIGHTS_PATH = Path(__file__).with_name("rl_weights.npz") if ROOT else Path("rl_weights.npz")
RL_ENABLED = os.environ.get("ARCHALUDON_RL_DISABLE", "0") != "1"
RL_TRAINING = os.environ.get("ARCHALUDON_RL_TRAIN", "0") == "1"
_rl_weights = None
_rl_load_error = None
...
def _agent_local_path(filename):
    candidates = []
    if ROOT:
        candidates.append(Path(ROOT).with_name(filename))
    candidates.append(Path.cwd() / filename)
    candidates.append(Path("/kaggle_simulations/agent") / filename)
...
def option_card(obs, opt):
    yi = obs.current.yourIndex
    opt_player = getattr(opt, "playerIndex", None)
    pi = opt_player if opt_player is not None else yi
...
def option_target(obs, opt):
    ...
    if in_play_area is None or in_play_index is None:
        return None
    opt_player = getattr(opt, "playerIndex", None)
    pi = opt_player if opt_player is not None else obs.current.yourIndex
    return get_card(obs, in_play_area, in_play_index, pi)
...
def _norm(value, scale, lo=-5.0, hi=5.0):
    try:
        return _clip(float(value) / float(scale), lo, hi)
    except Exception:
        return 0.0


def _one_hot(slot_count, index):
...
def _card_type_flags(card_id):
    data = CARD_DB.get(card_id)
    ctype = getattr(data, "cardType", None) if data else None
    ctype_value = _enum_index(ctype)
    return [
        _bool(ctype_value == 0),
        _bool(ctype_value == 1),
        _bool(ctype_value == 2),
        _bool(ctype_value == 3),
        _bool(ctype_value == 4),
        _bool(ctype_value in {5, 6}),
    ]


def _pokemon_scalar_features(pokemon):
    if pokemon is None:
        return [0.0] * 12
    max_hp = getattr(pokemon, "maxHp", getattr(pokemon, "hp", 0)) or 0
    hp = getattr(pokemon, "hp", 0) or 0
    return [
        _bool(pokemon.id == CINDERACE),
        _bool(pokemon.id == DURALUDON),
        _bool(pokemon.id == ARCHALUDON_EX),
        _bool(pokemon.id == RELICANTH),
        _norm(hp, 400),
        _norm(max_hp, 400),
        _norm(max(0, max_hp - hp), 400),
        _norm(energy_count(pokemon), 5),
        _bool(has_tool(pokemon)),
        _norm(retreat_cost(pokemon), 4),
        _norm(prize_value(pokemon), 3),
        _bool(hp <= opp_max_damage_safe(pokemon)),
    ]


def opp_max_damage_safe(_pokemon=None):
    # Used by feature extraction paths that should never fail on partial fake observations.
    return 220
...
def extract_state_features(obs):
    ...
    my_pokemon = all_my_pokemon(obs)
    bench = [p for p in (ps.bench or []) if p]
    opp_bench = opp_bench_pokemon(obs)
    matchup = detect_matchup(obs)
    route = archaludon_ex_attack_route(obs)
    attacks = planned_archaludon_attacks(obs)
    boss_ko_targets = 0
    boss_win_targets = 0
    remaining_prizes = len(ps.prize or [])
    for target in opp_bench:
        if any(effective_damage(atk["damage"], target) >= target.hp for atk in attacks):
            boss_ko_targets += 1
            if prize_value(target) >= remaining_prizes:
                boss_win_targets += 1
    opp_bench_remaining_hp = [getattr(p, "hp", 0) for p in opp_bench if p]
    opp_bench_prizes = [prize_value(p) for p in opp_bench if p]
    hand = ps.hand or []
    stadium_ids = [getattr(card, "id", None) for card in (obs.current.stadium or []) if card]
    features = [
        _norm(getattr(obs.current, "turn", 0), 30),
        _norm(getattr(obs.current, "turnActionCount", 0), 40),
        _norm(len(ps.prize or []), 6),
        _norm(len(opp.prize or []), 6),
        _norm(getattr(ps, "deckCount", 0), 60),
        _norm(getattr(opp, "deckCount", 0), 60),
        _norm(getattr(ps, "handCount", len(hand)), 20),
        _norm(getattr(opp, "handCount", 0), 20),
        _norm(metal_in_discard(obs), 12),
        _norm(sum(1 for c in hand if c and c.id == METAL_ENERGY), 8),
        _bool(getattr(obs.current, "supporterPlayed", False)),
        _bool(getattr(obs.current, "energyAttached", False)),
        _bool(getattr(obs.current, "retreated", False)),
        _bool(FULL_METAL_LAB in stadium_ids),
    ]
    features.extend(_one_hot(5, ["generic", "crustle", "hop", "lucario", "starmie"].index(matchup) if matchup in {"generic", "crustle", "hop", "lucario", "starmie"} else 0))
    features.extend(_pokemon_scalar_features(active))
    features.extend([
        _norm(len(bench), 5),
        _norm(sum(1 for p in bench if p.id == DURALUDON), 4),
        _norm(sum(1 for p in bench if p.id == ARCHALUDON_EX), 4),
        _norm(sum(1 for p in bench if p.id == CINDERACE), 2),
        _bool(any(p.id == RELICANTH for p in bench)),
        _bool(any(can_evolve_to_archaludon_now(p, obs) for p in bench)),
        _bool(any(p.id == DURALUDON and energy_count(p) >= 2 for p in bench)),
        _bool(any(p.id in {DURALUDON, ARCHALUDON_EX} and energy_count(p) >= 3 for p in my_pokemon)),
        _bool(sum(1 for p in my_pokemon if p.id in {DURALUDON, ARCHALUDON_EX}) >= 2),
        _bool(any(p.id == DURALUDON and energy_count(p) == 0 for p in my_pokemon)),
        _bool(any(p.id in {DURALUDON, ARCHALUDON_EX} and damage_on(p) >= 120 for p in my_pokemon)),
    ])
    features.extend(_pokemon_scalar_features(opp_active))
    features.extend([
        _norm(len(opp_bench), 5),
        _norm(min(opp_bench_remaining_hp) if opp_bench_remaining_hp else 0, 400),
        _norm(max(opp_bench_prizes) if opp_bench_prizes else 0, 3),
        _bool(boss_ko_targets),
        _bool(any(energy_count(p) >= 3 for p in opp_bench)),
        _bool(any(p.id == HOP_SNORLAX for p in opp_bench)),
        _bool(any(p.id in CRUSTLE_LINE for p in [opp_active] + opp_bench if p)),
        _bool(any(p.id in LUCARIO_LINE for p in [opp_active] + opp_bench if p)),
    ])
    active_attacks = active and attack_energy_route(obs, active)[0]
    active_attack_kos = bool(
        opp_active and attacks and any(effective_damage(atk["damage"], opp_active) >= opp_active.hp for atk in attacks)
    )
    features.extend([
        _bool(active and active.id == ARCHALUDON_EX and energy_count(active) >= 3),
        _bool(active and active.id in {DURALUDON, ARCHALUDON_EX} and energy_count(active) >= 3),
        _bool(active_attacks),
        _bool(route and route.get("uses_attach")),
        _bool(route and route.get("needs_retreat")),
        _bool(active_attack_kos),
        _bool(boss_ko_targets),
        _bool(boss_win_targets),
        _bool(active and has_in_play(obs, RELICANTH) and damage_on(active) >= 120),
        _bool(any(p.id in {DURALUDON, ARCHALUDON_EX} and energy_count(p) >= 3 for p in my_pokemon)),
    ])
    return _pad_features(features, 96)


def extract_option_features(obs, opt, rule_score, reason):
    card = option_card(obs, opt)
    target = option_target(obs, opt)
    cid = card.id if card else getattr(opt, "cardId", None)
    tid = target.id if target else None
    ...
    ctx = obs.select.context
    target_player = getattr(opt, "playerIndex", obs.current.yourIndex)
    attack_id = getattr(opt, "attackId", None)
    attack_damage = best_attack_damage(obs, attack_id) if attack_id is not None else 0
    opp_active = opp_active_pokemon(obs)
    option_flags = [
        _norm(rule_score, 10000),
        _norm(_clip(rule_score, -20000, 30000), 10000),
        _bool(opt_type == OptionType.PLAY),
        _bool(opt_type == OptionType.EVOLVE),
        _bool(opt_type == OptionType.ATTACH),
        _bool(opt_type == OptionType.ATTACK),
        _bool(opt_type in {OptionType.CARD, OptionType.DISCARD, OptionType.TOOL_CARD, OptionType.ENERGY_CARD}),
        _bool(ctx == SelectContext.TO_HAND),
        _bool(ctx in {SelectContext.SWITCH, SelectContext.TO_ACTIVE}),
        _bool(ctx == SelectContext.HEAL),
        _bool(ctx == SelectContext.DAMAGE),
        _norm(getattr(opt, "number", 0) or 0, 10),
    ]
    card_flags = [
        _bool(cid == DURALUDON),
        _bool(cid == ARCHALUDON_EX),
        _bool(cid == CINDERACE),
        _bool(cid == RELICANTH),
        _bool(cid == METAL_ENERGY),
        _bool(cid == ULTRA_BALL),
        _bool(cid == POKEGEAR),
        _bool(cid == NIGHT_STRETCHER),
        _bool(cid == JUMBO_ICE_CREAM),
        _bool(cid == HERO_CAPE),
        _bool(cid == FULL_METAL_LAB),
        _bool(cid == BOSS),
        _bool(cid == EXPLORER),
        _bool(cid == LILLIE),
    ]
    if cid is not None:
        card_flags.extend(_card_type_flags(cid))
    else:
        card_flags.extend([0.0] * 6)
    target_flags = [
        _bool(target is not None and target_player == obs.current.yourIndex),
        _bool(target is not None and target_player != obs.current.yourIndex),
        _bool(getattr(opt, "inPlayArea", None) == AreaType.ACTIVE),
        _bool(getattr(opt, "inPlayArea", None) == AreaType.BENCH),
        _bool(tid == DURALUDON),
        _bool(tid == ARCHALUDON_EX),
        _bool(tid == CINDERACE),
        _bool(tid == HOP_SNORLAX),
    ]
    if target is not None:
        target_flags.extend([
            _norm(getattr(target, "hp", 0), 400),
            _norm(max(0, getattr(target, "maxHp", getattr(target, "hp", 0)) - getattr(target, "hp", 0)), 400),
            _norm(energy_count(target), 5),
            _norm(prize_value(target), 3),
            _bool(attack_damage and effective_damage(attack_damage, target) >= target.hp),
            _bool(target_player != obs.current.yourIndex and prize_value(target) >= 2),
            _bool(target_player == obs.current.yourIndex and damage_on(target) >= 120),
        ])
    else:
        target_flags.extend([0.0] * 7)
    attack_flags = [
        _norm(attack_damage, 300),
        _bool(opp_active and attack_damage and effective_damage(attack_damage, opp_active) >= opp_active.hp),
        _bool(opp_active and attack_damage and effective_damage(attack_damage, opp_active) >= opp_active.hp and prize_value(opp_active) >= len(my_state(obs).prize or [])),
        _bool(attack_id == RAGING_HAMMER),
        _bool(attack_id == METAL_DEFENDER),
        _bool(detect_matchup(obs) == "crustle" and attack_id == METAL_DEFENDER),
    ]
    reason_flags = [
        _bool("Boss" in str(reason)),
        _bool("Explorer" in str(reason)),
        _bool("UB:" in str(reason)),
        _bool("Crustle" in str(reason)),
        _bool("hard:" in str(reason)),
    ]
    features = []
    features.extend(_one_hot(49, _enum_index(ctx)))
    features.extend(_one_hot(17, _enum_index(opt_type)))
    features.extend(option_flags)
    features.extend(card_flags)
    features.extend(target_flags)
    features.extend(attack_flags)
    features.extend(reason_flags)
    return _pad_features(features, 96)


def _pad_features(values, size):
...
def extract_features(obs, opt, rule_score, reason):
    return _pad_features(extract_state_features(obs) + extract_option_features(obs, opt, rule_score, reason), FEATURE_DIM)


def _load_rl_weights():
    global _rl_weights, _rl_load_error
    if _rl_weights is not None:
        return _rl_weights
    _rl_weights = {}
    ...
    try:
        import numpy as np
        data = np.load(RL_WEIGHTS_PATH, allow_pickle=False)
        _rl_weights = {key: data[key].astype(float).tolist() for key in data.files}
    except Exception as exc:
        _rl_load_error = f"{type(exc).__name__}: {exc}"
        _rl_weights = {}
    return _rl_weights


def reload_rl_weights():
    global _rl_weights, _rl_load_error
    _rl_weights = None
    _rl_load_error = None
    return _load_rl_weights()


def _linear(vec, weights, bias):
    return [sum(v * row_i for v, row_i in zip(vec, row)) + b for row, b in zip(weights, bias)]
...
def rl_model_forward(features):
    weights = _load_rl_weights()
    required = ["w1", "b1", "w2", "b2", "w_delta", "b_delta"]
    if not RL_ENABLED or not all(key in weights for key in required):
        return 0.0
    try:
        h1 = _relu(_linear(features, weights["w1"], weights["b1"]))
        h2 = _relu(_linear(h1, weights["w2"], weights["b2"]))
        out = _linear(h2, weights["w_delta"], weights["b_delta"])[0]
        return _tanh(out)
    except Exception as exc:
        global _rl_load_error
        _rl_load_error = f"{type(exc).__name__}: {exc}"
        return 0.0


def context_delta_limit(obs, opt):
    ctx = obs.select.context
    opt_type = getattr(opt, "type", None)
    if ctx in {SelectContext.IS_FIRST, SelectContext.MULLIGAN,
               SelectContext.SETUP_ACTIVE_POKEMON, SelectContext.SETUP_BENCH_POKEMON}:
        return 0.0
    if opt_type == OptionType.ATTACK or ctx == SelectContext.ATTACK:
        return 250.0
    if opt_type == OptionType.EVOLVE or ctx in {SelectContext.EVOLVE, SelectContext.EVOLVES_TO, SelectContext.EVOLVES_FROM}:
        return 6000.0
    if opt_type == OptionType.ATTACH or ctx in {SelectContext.ATTACH_TO, SelectContext.ATTACH_FROM}:
        return 5000.0
    ...
    if ctx in {SelectContext.DISCARD, SelectContext.DISCARD_CARD_OR_ATTACHED_CARD}:
        return 8000.0
    if ctx in {SelectContext.SWITCH, SelectContext.TO_ACTIVE}:
        target = option_card(obs, opt)
        pi = getattr(opt, "playerIndex", obs.current.yourIndex)
        return 8000.0 if pi != obs.current.yourIndex and target else 4000.0
    if ctx == SelectContext.HEAL:
        return 5000.0
...
def _delta_mode(obs, opt):
    ctx = obs.select.context
    opt_type = getattr(opt, "type", None)
    card = option_card(obs, opt)
    cid = card.id if card else getattr(opt, "cardId", None)
    if ctx in {SelectContext.HEAL, SelectContext.SWITCH, SelectContext.TO_ACTIVE, SelectContext.ATTACH_TO, SelectContext.ATTACH_FROM}:
        return "full"
    if opt_type in {OptionType.RETREAT, OptionType.EVOLVE, OptionType.ATTACH}:
        return "full"
    if ctx in {SelectContext.TO_HAND, SelectContext.DISCARD, SelectContext.DISCARD_CARD_OR_ATTACHED_CARD}:
        return "down_only"
    if ctx == SelectContext.MAIN and cid in {EXPLORER, LILLIE, POKEGEAR}:
        return "down_only"
    return "off"


def scale_and_clip_delta(obs, opt, raw_delta):
    limit = context_delta_limit(obs, opt)
    return _clip(float(raw_delta) * limit, -limit, limit)


def apply_hard_guard(obs, opt, rule_score, delta):
    if rule_score <= -9000:
        return float(rule_score), 0.0
    limit = context_delta_limit(obs, opt)
    delta = _clip(delta, -limit, limit)
    mode = _delta_mode(obs, opt)
    if mode == "off":
        return float(rule_score), 0.0
    if mode == "down_only":
        delta = min(delta, 0.0)
    card = option_card(obs, opt)
    cid = card.id if card else getattr(opt, "cardId", None)
    if getattr(my_state(obs), "deckCount", 99) <= 5 and cid in {EXPLORER, LILLIE}:
        return float(min(rule_score, -5000)), 0.0
    if opt.type == OptionType.RETREAT and rule_score < 0:
        delta = min(delta, 0.0)
    if cid == BOSS and not planned_archaludon_attacks(obs) and rule_score < 0:
        delta = min(delta, 0.0)
    if obs.select.context == SelectContext.HEAL:
        active = active_pokemon(obs)
        if active:
            skip, _ = should_skip_ice_cream(obs, active)
            if skip and rule_score < 0:
                delta = min(delta, 0.0)
    return float(rule_score) + float(delta), float(delta)


def final_score_option(obs, opt):
    rule_score, reason = score_option(obs, opt)
    features = extract_features(obs, opt, rule_score, reason)
    raw_delta = rl_model_forward(features)
    delta = scale_and_clip_delta(obs, opt, raw_delta)
    final_score, delta = apply_hard_guard(obs, opt, float(rule_score), delta)
    return float(final_score), float(rule_score), float(delta), reason


def score_options_with_debug(obs):
    scored = []
    debug_options = []
    for i, opt in enumerate(obs.select.option):
        try:
            final_score, rule_score, delta, reason = final_score_option(obs, opt)
            features = extract_features(obs, opt, rule_score, reason)
        except Exception as e:
            final_score, rule_score, delta, reason = -999999.0, -999999.0, 0.0, f"error {type(e).__name__}: {e}"
            features = [0.0] * FEATURE_DIM
        scored.append((final_score, i, rule_score, delta, reason, features))
        debug_options.append({
            ...
            "delta": float(delta),
            "reason": str(reason),
            "option_type": getattr(getattr(opt, "type", None), "name", str(getattr(opt, "type", None))),
            "card_id": getattr(option_card(obs, opt), "id", getattr(opt, "cardId", None)),
            "feature_dim": len(features),
        })
    scored.sort(key=lambda x: (x[0], -x[1]), reverse=True)
    return scored, debug_options


def _select_from_scored(obs, scored):
    selected = []
    for final_score, i, _rule_score, _delta, _reason, _features in scored:
        if len(selected) >= obs.select.maxCount:
            break
        if final_score < 0 and len(selected) >= obs.select.minCount:
            continue
        selected.append(i)
...
def _context_temperature(obs):
    ...
    if ctx in {SelectContext.TO_HAND, SelectContext.DISCARD, SelectContext.DISCARD_CARD_OR_ATTACHED_CARD}:
        return 0.70
    if ctx in {SelectContext.HEAL, SelectContext.SWITCH, SelectContext.TO_ACTIVE}:
        return 0.50
    return 0.50


def _candidate_scored(obs, scored):
    viable = [item for item in scored if item[2] > -9000]
    if not viable:
        viable = list(scored)
    top = viable[0][0] if viable else 0.0
    margin = 8000.0
    limited = [item for item in viable if top - item[0] <= margin]
    return limited[:_context_top_k(obs)] or viable[:1]


def _softmax_sample(scored, temperature):
    import math
    scale = max(1e-6, temperature * 10000.0)
    logits = [item[0] / scale for item in scored]
    max_logit = max(logits)
    weights = [math.exp(_clip(logit - max_logit, -50, 50)) for logit in logits]
    total = sum(weights)
...
def _gumbel_topk(scored, count, temperature):
    import math
    scale = max(1e-6, temperature * 10000.0)
    noisy = []
    for item in scored:
        u = _clip(random.random(), 1e-9, 1.0 - 1e-9)
        g = -math.log(-math.log(u))
        noisy.append((item[0] + g * scale, item[1]))
    noisy.sort(reverse=True)
    return [idx for _, idx in noisy[:count]]


def _record_decision(obs, debug_options, selected):
    record = {
        "player": obs.current.yourIndex,
        "turn": getattr(obs.current, "turn", None),
        "context": getattr(obs.select.context, "name", str(obs.select.context)),
    ...
    for item in debug_options:
        option = obs.select.option[item["option_index"]]
        rule_score = item["rule_score"]
        reason = item["reason"]
        record["options"].append({
            **item,
            "selected": item["option_index"] in selected_set,
            "option_features": extract_option_features(obs, option, rule_score, reason),
        })
    _training_decisions.append(record)
    return record


def choose_options_train(obs):
    scored, debug_options = score_options_with_debug(obs)
    greedy_selected = _select_from_scored(obs, scored)
    target_count = len(greedy_selected)
    if target_count == 0 and obs.select.minCount == 0:
        selected = []
        _record_decision(obs, debug_options, selected)
        return selected
    candidates = _candidate_scored(obs, scored)
    temperature = _context_temperature(obs)
    if obs.select.maxCount <= 1:
        selected = [_softmax_sample(candidates, temperature)]
    else:
        target_count = max(obs.select.minCount, min(target_count, obs.select.maxCount))
        if target_count <= 0:
            selected = []
        else:
            selected = _gumbel_topk(candidates, target_count, temperature)
        if len(selected) < obs.select.minCount:
            selected = _select_from_scored(obs, scored)
    _record_decision(obs, debug_options, selected)
    return selected


def get_last_decision_debug():
    return dict(_last_decision_debug)


def consume_training_decisions():
    rows = list(_training_decisions)
    _training_decisions.clear()
    return rows
...
def choose_options(obs):
    ...
    _last_decision_debug = {
        ...
        "weights_loaded": bool(_load_rl_weights()),
        "weights_error": _rl_load_error,
    }
    return selected
...
def agent(obs_dict):
    ...
    try:
        if RL_TRAINING:
            return choose_options_train(obs)
